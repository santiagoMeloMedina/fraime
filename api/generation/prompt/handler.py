from api.generation.prompt.model import (
...
class PromptHandler:
    ...
    def compile(video_type: str, fields: dict) -> str:
        ...
        if isinstance(prompt, MusicVideoPromptComponents):
            return PromptHandler._compile_music_video(prompt)
        if isinstance(prompt, MotionGraphicsPromptComponents):
            return PromptHandler._compile_motion_graphics(prompt)
        if isinstance(prompt, UGCPromptComponents):
            return PromptHandler._compile_dialogue_driven(prompt)
        return PromptHandler._compile_cinematic(prompt)
